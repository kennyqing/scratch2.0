...
//-------- recording video code ---------
	...
	public function saveRecording():void {
		...
		//ExternalInterface.call("uploadVideo",video);
		//ExternalInterface.call("test",video.length);
		sendVideoData(video,"testuploadvideo");
...
//		DialogBox.close("Video Finished!","To save, click the button below.",null,"Save and Download",app.stage,saveFile,releaseVideo,null,true);
	...
	protected function sendVideoData(data:ByteArray,fileName:String):void {
		trace("上传的视频数据："+data);
		ExternalInterface.call("test","进入上传视频方法：sendVideoData");
		var url:String="http://localhost:8081/youpinwxshare/uploadVideo1"
		...
		//uploadRequest.contentType = "application/octet-stream;boundary="+UploadPostHelper.getBoundary();
		...
		ExternalInterface.call("test","dddddd");

		var uploader:URLLoader = new URLLoader();
		uploader.addEventListener(ProgressEvent.PROGRESS, onUploadVideoProgress);
		...
		uploader.addEventListener(SecurityErrorEvent.SECURITY_ERROR, uploadVideoSecurityError)
		uploader.dataFormat = URLLoaderDataFormat.BINARY;
		ExternalInterface.call("test","bbbbbb");
		//uploader.load(uploadRequest);

		trace("测试dfgdgdgd"+uploadRequest);

		uploader.load(uploadRequest);

		trace("什么情况");

		ExternalInterface.call("test","什么情况");
	}

	private function onUploadVideoProgress(e:ProgressEvent){
		ExternalInterface.call("test","onUploadVideoProgress");
	}

	private function uploadVideoSecurityError(e:SecurityErrorEvent){
		ExternalInterface.call("test","出错uploadVideoSecurityError");
	}

	private function handleUploadVideoComplete( e:Event ):void {
		ExternalInterface.call("test","进入上传视频完成方法handleUploadVideoComplete");
		var loader:URLLoader = URLLoader(e.target);
		var jsonString:String=e.target.data as String;
		var jsonString:String=loader.data.toString();
		ExternalInterface.call("test",jsonString);
...
//----------
	...
	public function loadProjectFromCloud(originalName:String,newName:String):void{
		ExternalInterface.call("test","loadProjectFromCloud:"+newName);
		var fileName:String,data:ByteArray;
		function fileLoadHandler(event:Event):void {
			ExternalInterface.call("test","----->fileLoadHandler");
		...
		function doInstall(ignore:* = null):void {
			installProjectFromServer(fileName, data);
		}
		stopAll();

		var uploadRequest:URLRequest = new URLRequest("http://localhost:8081/upLoadFile/"+newName);
		uploadRequest.method = URLRequestMethod.POST;

		var uploader:URLLoader = new URLLoader;
		uploader.addEventListener(Event.COMPLETE, fileLoadHandler);
		uploader.addEventListener(IOErrorEvent.IO_ERROR, this.ioErrorHandler);
		uploader.dataFormat = URLLoaderDataFormat.BINARY;
		uploader.load(uploadRequest);

		//Scratch.loadSingleFileFromServer(fileLoadHandler,selectFileName);
	}

	private function ioErrorHandler(e:IOErrorEvent):void
	{
		ExternalInterface.call("test","上传视频出错"+e.toString());
		trace(e.toString());
	}

	public function installProjectFromServer(fileName:String, data:ByteArray):void {
		// Install a project from a file with the given name and contents.
		stopAll();
		app.oldWebsiteURL = '';
		app.loadInProgress = true;
		installProjectFromServerData(data);
		app.setProjectName(fileName);
	}

	public function installProjectFromServerData(data:ByteArray, saveForRevert:Boolean = true):void {
