	...
	protected function determineJSAccess():void {
		if (externalInterfaceAvailable()) {
			try {
				...
				//externalCall("receiveProjectName",receiveProjectName);
				ExternalInterface.addCallback("receiveProjectName",receiveProjectName);
				ExternalInterface.addCallback("loadProjectFromServer",loadProjectFromServer);
				return; // wait for callback
			}
			catch (e:Error) {
			}
		}
		jsAccessDetermined(true);
	}
...
//		}
	...
	public function showFileMenu(b:*):void {
		...
		m.addItem('从云端载入项目',selectLoadProJS);
		m.addLine();
		m.addItem('保存项目到云端',saveProjectToCloud);
		m.addLine();
		//m.addItem('打开本地文件',openLocalProject);
		//m.addLine();
		m.addItem('邀请好友一起玩',inviteFriendPlayer);
		m.addLine();

		m.addItem('测试上传视频',testUpLoadVideo);
		m.addLine();
	...
	protected function addFileMenuItems(b:*, m:Menu):void {
		...
		m.addItem('打开本地文件', runtime.selectProjectFile);
	...
	protected function saveProjectToCloudAndThen(callback:Function = null):void {
		saveProToCloudAndThen();
	}
	...
	//把项目保存到云端
	protected function saveProjectToCloud(ignore:* = null):void {
		saveProjectToCloudAndThen();
	}

	//选择要从云端加载项目,调用前端js选择项目
	protected function selectLoadProJS(ignore:* = null):void {
		ExternalInterface.call("selectLoadProName");
		//ExternalInterface.call("test","kenny");
	}

	//接收前端传过来的要加载的项目名称，从云端加载项目
	private function loadProjectFromServer(originalName:String,newName:String):void{
		ExternalInterface.call("test",originalName);
		runtime.loadProjectFromCloud(originalName,newName);
	}

	private var uploadURL:URLRequest;
	private var file:FileReference;
	private function testUpLoadVideo(ignore:* = null){
		uploadURL= new URLRequest();
		uploadURL.url = "http://localhost:8081/youpinwxshare/uploadVideo";
	...
	private function selectHandler(event:Event):void {
		...
		//uploadURL.data=UploadPostHelper.getPostData(file.name,event.target.data);
		uploadURL.data=UploadPostHelper.getPostData(file.name,event.target.data);

		var uploader:URLLoader = new URLLoader();
		//uploader.addEventListener(Event.COMPLETE, handleUploadVideoComplete);
		uploader.addEventListener(IOErrorEvent.IO_ERROR, this.ioErrorHandler);
		uploader.dataFormat = URLLoaderDataFormat.BINARY;
		ExternalInterface.call("test","bbbbbb");
		//uploader.load(uploadRequest);

		uploader.load(uploadURL);
	...
	//打开本地项目
//	protected function openLocalProject(ignore:* = null):void {
//		//createNewProjectAndThen();
//		runtime.selectProjectFile
//	}

	//邀请好友一起玩
	protected function inviteFriendPlayer(ignore:* = null):void {
		createNewProjectAndThen();
	}
	...
	/***
	 * 把项目保存到云端
	 * @param postSaveAction
	 */
	protected function saveProToCloudAndThen(postSaveAction:Function = null):void {
		// Give the user a chance to save their project, if needed, then call postSaveAction.
		function doNothing():void {
		}
		...
		function save():void {
			...
			//sendData();
			//RresetProjectName();
			//ExternalInterface.call("inputProjectName");
		}
		...
		d.addTitle('保存项目到云端?');
		d.addButton('保存', save);
		d.addButton('不保存', proceedWithoutSaving);
		d.addButton('取消', cancel);
		d.showOnStage(stage);
	}

	private function receiveProjectName(inputName:String):String{
		trace("输入的项目名称是："+inputName);

		setProjectName(inputName);

//		var request:URLRequest=new URLRequest("http://localhost:8081/youpinwxshare/recevieProName");
//		var param:URLVariables=new URLVariables();
//		param.projectName=inputName;
//		request.data=param;
//
//		sendToURL(request);

		return "哈哈哈哈哈哈哈哈哈哈哈:"+inputName;

//		var d:DialogBox = new DialogBox();
//		d.addTitle(inputName);
//		d.showOnStage(stage);
		//Alert.show(message);
//		var result:String=inputName;
//        var test:TextField=new TextField();
//        test.text=result;
//		this.addChild(test);
		//return "kkkk:"+inputName;
	}

	private function RresetProjectName():void{
		ExternalInterface.call("test","testcalljs");
	}

	protected function sendData(data:ByteArray,fileName:String):void {
		ExternalInterface.call("test","进入sendData方法");
		var url:String="http://localhost:8081/youpinwxshare/testHandler"
		//var url:String="http://192.168.1.131:50002/subProgApi/public/upload/image"
		//var url:String="http://localhost:50002/subProgApi/public/upload/image"

		var uploadRequest:URLRequest = new URLRequest(url);
		uploadRequest.method = URLRequestMethod.POST;
		uploadRequest.contentType = "multipart/form-data;boundary="+UploadPostHelper.getBoundary();
		//uploadRequest.contentType = "multipart/form-data";
		uploadRequest.data=UploadPostHelper.getPostData(fileName,data);

		//ExternalInterface.call("test",data);

		var uploader:URLLoader = new URLLoader;
		//uploader.addEventListener(ProgressEvent.PROGRESS, onUploadProgress);
		uploader.addEventListener(Event.COMPLETE, handleComplete);
		uploader.addEventListener(IOErrorEvent.IO_ERROR, this.ioErrorHandler);
		uploader.dataFormat = URLLoaderDataFormat.BINARY;
		//uploader.load(uploadRequest);
		uploader.load(uploadRequest);
	...
	private function handleComplete( e:Event ):void {
		...
		var jsonString:String=loader.data.toString();
		//ExternalInterface.call("test","kkkk");

		var result:Object=new Object();
		result=serialization.json.JSON.decode(jsonString);

		//ExternalInterface.call("test",result.map.newFileName);
		
		ExternalInterface.call("inputProjectName",result.code,result.map.newFileName,result.map.flag);
	...
	public function exportProjectToFile(fromJS:Boolean = false, saveCallback:Function = null):void {
		function squeakSoundsConverted():void {
			...
			//trace(zipData);
			sendData(zipData,fixFileName(defaultName));
		}
	...
	static public function loadSingleFileFromServer(fileLoaded:Function,selectFileName:String):void{
		ExternalInterface.call("test","loadSingleFileFromServer哈哈哈:"+selectFileName);
		if (selectFileName!=null && selectFileName!="") {
			ExternalInterface.call("test","selectFileName不为空");
			//var request:URLRequest=new URLRequest("http://localhost:8081/upLoadFile/"+selectFileName);
//			request.method=URLRequestMethod.POST;
//			var urlLoad:URLLoader=new URLLoader();
//			urlLoad.addEventListener(Event.COMPLETE, fileLoaded);
//			urlLoad.load(request);
					
			var file:FileReference =FileReference("http://localhost:8081/upLoadFile/"+selectFileName);
			file.addEventListener(Event.COMPLETE, fileLoaded);
			file.addEventListener(IOErrorEvent.IO_ERROR, handlerIoError);
			file.load();
		}
	}

	static public function handlerIoError(e:IOErrorEvent){
		ExternalInterface.call("test","出错信息："+e.toString());
	}
	...
	public static var SwfHomeUrl:String;
	public function GetHomeDir():String{
		...
		if(doMainArray[0]=="file:"){
			if(doMainArray.length<=3){
				SwfHomeUrl=doMainArray[2];
				SwfHomeUrl=SwfHomeUrl.substring(0,SwfHomeUrl.lastIndexOf(SwfHomeUrl.charAt(2)));
			}else {
				SwfHomeUrl=doMain;
				SwfHomeUrl=SwfHomeUrl.substring(0,SwfHomeUrl.lastIndexOf("/"));
			}
		}else{
			SwfHomeUrl=doMain;
			SwfHomeUrl=SwfHomeUrl.substring(0,SwfHomeUrl.lastIndexOf("/"));
		}
		SwfHomeUrl=SwfHomeUrl+"/";
